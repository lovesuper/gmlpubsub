...
function pubsub_subscribe(_event, _func) {
  global.PubSub.subscribe(other.id, _event, _func)
}

function pubsub_unsubscribe(_event) {
  global.PubSub.unsubscribe(other.id, _event)
}

function pubsub_unsubscribe_all() {
  global.PubSub.unsubscribe_all(other.id)
}

function pubsub_publish(_event, _data) {
  global.PubSub.publish(_event, _data)
}
